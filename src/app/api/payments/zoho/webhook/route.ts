...
export async function POST(request: NextRequest) {
  ...
  const signature =
    ...
    request.headers.get('x-zoho-payments-signature');
  ...
  if (confirmed) {
    ...
    if (paymentOrder.applicationId) {
      await db.agency.update({
        where: { id: paymentOrder.agencyId },
        data: {
          totalRevenueMinor: { increment: BigInt(paymentOrder.amountMinor) },
      ...
      await queueTravelAgentActivitySync({
        ...
        description: `Payment of ₹${(paymentOrder.amountMinor / 100).toLocaleString('en-IN')} confirmed via Zoho Payments for visa application.`,
      }).catch(() => null);
    } else {
      await queueTravelAgentActivitySync({
        agencyId: paymentOrder.agencyId,
        activityType: 'PAYMENT_COMPLETED',
        description: `Wallet top-up of ₹${(paymentOrder.amountMinor / 100).toLocaleString('en-IN')} confirmed via Zoho Payments.`,
      }).catch(() => null);
    }
    after(async () => {
      try {
