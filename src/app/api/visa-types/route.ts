...
export async function GET(request: NextRequest) {
  ...
  try {
    const [visaProducts, destinations, categoriesGroup] = await Promise.all([
    ...
    const filteredVisaProducts = visaProducts.filter(product => {
      if (!product.category) return true;
      return !disabled.includes(product.category);
    });

    const activeCategories = categoriesGroup
      .filter((c) => c.category && !disabled.includes(c.category))
      .map((c) => ({
        name: c.category,
        count: c._count.id,
      }));
    ...
    return NextResponse.json({
      visaTypes: filteredVisaProducts.map((product) => {
