...
export default function LoginView() {
  ...
  // Auto sign-in returning sessions if already authenticated with Firebase
  useEffect(() => {
    const unsubscribe = auth.onAuthStateChanged(async (user) => {
      ...
      try {
        ...
        if (onboardData.onboarded) {
          ...
          if (authRes.ok && data.agency) {
            login(data.agency);
            const targetSlug = data.agency.vvisaUid || data.agency.id;
            router.push(`/${targetSlug}/explore`);
          }
        }
      } catch (err) {
        console.error('Auto sign-in check error:', err);
      }
    });
