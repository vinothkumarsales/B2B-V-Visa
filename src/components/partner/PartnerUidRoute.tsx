...
export async function PartnerUidRoute({
  ...
  // Admin impersonation mode: show full partner portal with a persistent warning banner
  if (access.status === 'impersonating') {
    return (
      <>
        <ImpersonationBanner
          targetAgencyName={access.impersonation.targetAgencyName}
          targetVvisaUid={access.impersonation.targetVvisaUid}
          actorRole={access.impersonation.actorRole}
        />
