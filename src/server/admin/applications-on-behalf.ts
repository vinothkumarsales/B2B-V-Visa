...
export async function createApplicationDraftOnBehalf(input: { partnerUid: string; adminUid: string; payload: z.infer<typeof adminApplicationDraftSchema> }) {
  ...
  if (input.payload.supportSessionId) {
    await requireActiveSupportSession({ sessionId: input.payload.supportSessionId, adminUid: input.adminUid, partnerUid: partner.id, minimumMode: 'operations' });
  }
...
export async function submitApplicationOnBehalf(input: { applicationId: string; adminUid: string; payload: z.infer<typeof adminApplicationSubmitSchema> }) {
  ...
  const supportSessionId = input.payload.supportSessionId ?? application.adminSupportSessionId ?? undefined;
  if (supportSessionId) await requireActiveSupportSession({ sessionId: supportSessionId, adminUid: input.adminUid, partnerUid: application.agencyId, minimumMode: 'operations' });
