...
export interface ZohoPaymentSessionInput {
  ...
  transactionType?: 'WALLET_TOPUP' | 'APPLICATION_PAYMENT';
...
const liveZohoPaymentProvider: ZohoPaymentProvider = {
  ...
  async createSession(input) {
    ...
    const isTopup = input.transactionType === 'WALLET_TOPUP' || !input.applicationId;
    const refPrefix = isTopup ? 'TOPUP' : 'VTX';
    const uidTag = input.vvisaUid ? `-${input.vvisaUid}` : '';
    const referenceNumber = `${refPrefix}${uidTag}-${input.paymentOrderId.slice(-8)}`.toUpperCase();
    const description = isTopup
      ? `V-Visa Wallet Top-Up (${input.vvisaUid ?? input.agencyName})`
      : `V-Visa Application Payment (${input.vvisaUid ?? input.agencyName})`;

    const metaData = [
      { key: 'order_id', value: input.paymentOrderId },
      { key: 'ref_num', value: referenceNumber },
      { key: 'type', value: isTopup ? 'WALLET_TOPUP' : 'APPLICATION_PAYMENT' },
    ];
    if (input.vvisaUid) metaData.push({ key: 'vvisa_uid', value: input.vvisaUid });
    if (input.agencyId) metaData.push({ key: 'agency_id', value: input.agencyId });
    if (input.applicationId) metaData.push({ key: 'app_id', value: input.applicationId });
    ...
    const payload = {
      ...
      configurations: {
        hosted_checkout_parameters: {
          ...
          udf2: input.vvisaUid ?? input.agencyId ?? '',
          udf3: referenceNumber,
        },
      },
    };
  ...
  verifyWebhookSignature(rawBody, signature) {
    ...
    // Parse t=<timestamp> and v=<signature> from Zoho webhook signature header
    ...
    for (const part of parts) {
      ...
      else if (key === 'v') receivedSignature = val;
    }
