...
import { buildConfiguredPassportCrmUpdateFields, buildConfiguredTravelAgentCrmFields, buildConfiguredVisaInterestLeadCreateFields, convertZohoCrmLead, createZohoCrmNote, createZohoCrmRecord, findZohoCrmRecord, updateZohoCrmRecord } from './record-update';
  ...
  try {
    ...
    if (event.eventType === 'TRAVEL_AGENT_UPSERT' || event.eventType === 'TRAVEL_AGENT_PROFILE_UPDATED') {
      ...
      const fields = buildConfiguredTravelAgentCrmFields({
        portalTravelAgentId: payload.vvisaUid || payload.agencyId,
      ...
      const matchedByIdentity = !existingMapping && !payload.existingZohoRecordId
        ? await findFirstTravelAgentMatch(payload, payload.vvisaUid ? 'VVISA_UID' : 'UID')
        : null;
    ...
    if (event.eventType === 'TRAVEL_AGENT_ACTIVITY_SYNC') {
      const payload = event.payload as Record<string, unknown>;
      const agencyId = String(payload.agencyId || event.agencyId);
      const activityType = String(payload.activityType || 'ACTIVITY');
      const description = String(payload.description || 'Portal activity');
      const country = typeof payload.country === 'string' ? payload.country : null;
      const totalVisasSubmitted = typeof payload.totalVisasSubmitted === 'number' ? payload.totalVisasSubmitted : null;
      const totalRevenueMinor = typeof payload.totalRevenueMinor === 'string' ? payload.totalRevenueMinor : null;

      const existingMapping = await db.crmEntityMapping.findUnique({
        where: {
          portalEntityType_portalEntityId_zohoModule: {
            portalEntityType: 'Agency',
            portalEntityId: agencyId,
            zohoModule: env.ZOHO_CRM_TRAVEL_AGENTS_MODULE,
          },
        },
      });

      const zohoRecordId = existingMapping?.zohoRecordId || (typeof payload.existingZohoRecordId === 'string' ? payload.existingZohoRecordId : null);

      if (zohoRecordId && env.CRM_WRITE_ENABLED) {
        const updateFields: Record<string, string> = {};
        if (country) updateFields['last_txn_country_or_insurance'] = country;
        if (totalVisasSubmitted != null && totalVisasSubmitted > 0) updateFields['total_visa'] = String(totalVisasSubmitted);
        if (totalRevenueMinor != null && Number(totalRevenueMinor) > 0) updateFields['total_revenue'] = String(Number(totalRevenueMinor) / 100);
        updateFields['remarks'] = description;
        ...
        await updateZohoCrmRecord({
          ...
          console.warn('[CRM_WORKER] Travel Agent activity field update warning:', err);
        });

        await createZohoCrmNote({
          parentModule: env.ZOHO_CRM_TRAVEL_AGENTS_MODULE,
          parentId: zohoRecordId,
          title: `Partner Activity: ${activityType}`,
          content: `${description} | Timestamp: ${new Date().toISOString()}`,
        });
      }

      return markZohoCrmEventCompleted({
        eventId: event.id,
        providerRecordId: zohoRecordId ?? undefined,
      });
    }
    ...
    if (event.eventType === 'VISA_INTEREST_LEAD_CREATE') {
      ...
      // Find parent Travel Agent in Zoho to associate the Lead
      const travelAgentMapping = await db.crmEntityMapping.findUnique({
        where: {
          portalEntityType_portalEntityId_zohoModule: {
            portalEntityType: 'Agency',
            portalEntityId: event.agencyId,
            zohoModule: env.ZOHO_CRM_TRAVEL_AGENTS_MODULE,
          },
        },
      });
      ...
      const fields = buildConfiguredVisaInterestLeadCreateFields({
        ...
        travelAgentRecordId: travelAgentMapping?.zohoRecordId ?? null,
      });
    ...
    if (event.eventType === 'LEAD_CONVERT') {
      ...
      // Look up parent Travel Agent in Zoho
      const travelAgentMapping = await db.crmEntityMapping.findUnique({
        where: {
          portalEntityType_portalEntityId_zohoModule: {
            portalEntityType: 'Agency',
      ...
      if (travelAgentMapping?.zohoRecordId) {
        // Link Contact to parent Travel Agent
        await updateZohoCrmRecord({
          moduleApiName: env.ZOHO_CRM_CONTACTS_MODULE,
          recordId: converted.contactId,
          fields: {
            Travel_Agent: travelAgentMapping.zohoRecordId,
            Travel_Agents: travelAgentMapping.zohoRecordId,
          },
        }).catch(() => {});

        // Fetch application details to enrich Travel Agent master record
        const application = await db.visaApplication.findUnique({
          where: { id: payload.applicationId },
          select: { destination: true, visaType: true, totalAmountMinor: true },
        });

        const amount = application?.totalAmountMinor ? application.totalAmountMinor / 100 : 0;

        // Update Travel Agent metrics in Zoho CRM
        await updateZohoCrmRecord({
          moduleApiName: env.ZOHO_CRM_TRAVEL_AGENTS_MODULE,
          recordId: travelAgentMapping.zohoRecordId,
          fields: {
            last_txn_date: new Date().toISOString().split('T')[0],
            ...(application?.destination ? { last_txn_country_or_insurance: application.destination } : {}),
            remarks: `Converted from Lead. Application: ${application?.destination ?? 'Visa'} (${application?.visaType ?? 'Tourist'}). Paid: ₹${amount}`,
          },
        }).catch(() => {});

        // Create Zoho Timeline Note under Travel Agent
        await createZohoCrmNote({
          parentModule: env.ZOHO_CRM_TRAVEL_AGENTS_MODULE,
          parentId: travelAgentMapping.zohoRecordId,
          title: 'Payment Completed & Lead Converted',
          content: `Payment of ₹${amount} completed for ${application?.destination ?? 'Visa'} application. Lead converted to Contact (${converted.contactId}).`,
        });
      }

      // Update Agency local statistics
      await db.agency.update({
...
function parseTravelAgentPayload(value: unknown): TravelAgentPayload {
  ...
  return {
    ...
    vvisaUid: stringOrNull(payload.vvisaUid),
    agencyName: payload.agencyName,
