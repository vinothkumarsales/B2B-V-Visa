...
export async function convertZohoCrmLead(leadId: string) {
  ...
  const contactsRaw = first?.Contacts ?? first?.details?.Contacts;
  const contactId = typeof contactsRaw === 'string' 
    ? contactsRaw 
    : (typeof contactsRaw === 'object' && contactsRaw !== null && 'id' in contactsRaw ? String((contactsRaw as Record<string, unknown>).id) : undefined);
  if (!contactId) throw validationError('ZOHO_CONTACT_ID_MISSING');
  return { contactId };
}

export async function createZohoCrmNote(input: {
  parentModule: string;
  parentId: string;
  title: string;
  content: string;
  ...
  try {
    const response = await zohoCrmFetch('/Notes', {
      method: 'POST',
      body: JSON.stringify({
        data: [
          {
    ...
    if (!response.ok) {
      console.warn(`[ZOHO_NOTE] Failed to create note on ${input.parentModule} (${response.status})`);
      return null;
    }
    const payload = (await response.json().catch(() => ({}))) as {
      data?: Array<{ details?: { id?: string } }>;
    };
    return payload.data?.[0]?.details?.id ?? null;
  } catch (err) {
    console.warn('[ZOHO_NOTE] Note creation error:', err);
    return null;
  }
}
